...
| F2 — Full | F1 + `type_of_violence` (one-hot) + `side_b` actor identity (one-hot, top-N by frequency; remaining actors collapsed into `other`) + `best` fatality estimate, min-max normalised.|
